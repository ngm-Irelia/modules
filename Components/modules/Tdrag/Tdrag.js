    ...
    jQuery(function() {
        ...
        Dragfn.prototype = {
            init: function (obj) {
				...
				//关闭按钮
				self.closebox = $.type(self.options.close)==="string" ? self.options.close : null;
               if($.type(self.options.close)==="string"){
				    $(self.options.close).off('click').on('click',function(){
						
						console.log($(this).parents(self.options.item)[0])
						self.delDrag($(this).parents(self.options.item)[0]);
					})
			   }
            ...
            delDrag:function(obj){
				var self=this;
				//先排序
				var arr_li=self.sort();  // 这个地方可能需要改下，去手动给最后一个元素添加 tezml
		
				//换位置
				var oNear;
				if (self.$element.length>0) {
					// 循环所有 ele，取得index值最大的那个
				    oNear = self.$element[self.$element.length-1];
					for(let ei = 0 ;ei<self.$element.length; ei++){
						if( self.$element[ei].getAttribute("index") > oNear.getAttribute("index")){
							oNear = self.$element[ei];
						}
					}
				}else{
					// 仅仅删除当前元素，然后return
