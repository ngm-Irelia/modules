		...
		return {
			circles: function() {
				...
				circles = function(sel) {
					...
					g = sel.append("defs").append("pattern").attr("id", id).attr("patternUnits", "userSpaceOnUse").attr("width",
						size).attr("height", size);
					if (background) {
						g.append("rect").attr("width", size).attr("height", size).attr("fill", background)
					}
					g.append("circle").attr("cx", size / 2).attr("cy", size / 2).attr("r", radius).attr("fill", fill).attr(
						"stroke", stroke).attr("stroke-width", strokeWidth);
					if (complement) {
						ref = [
							[0, 0],
							[0, size],
						...
						for (i = 0, len = ref.length; i < len; i++) {
							corner = ref[i];
							results.push(g.append("circle").attr("cx", corner[0]).attr("cy", corner[1]).attr("r", radius).attr("fill",
								fill).attr("stroke", stroke).attr("stroke-width", strokeWidth))
						}
						return results
					}
				};
			...
			lines: function() {
				...
				lines = function(sel) {
					...
					for (i = 0, len = orientation.length; i < len; i++) {
						o = orientation[i];
						results.push(g.append("path").attr("d", path(o)).attr("stroke-width", strokeWidth).attr("shape-rendering",
							shapeRendering).attr("stroke", stroke).attr("stroke-linecap", "square"))
					}
					return results
				};
			...
			paths: function() {
				...
				svgPath = function(_) {
					switch (_) {
						...
						case "nylon":
							return function(s) {
								return "M 0 " + s / 4 + " l " + s / 4 + " 0 l 0 " + -s / 4 + " M " + s * 3 / 4 + " " + s + " l 0 " + -s /
									4 + "\nl " + s / 4 + " 0 M " + s / 4 + " " + s / 2 + " l 0 " + s / 4 + " l " + s / 4 + " 0 M " + s / 2 +
									" " + s / 4 + "\nl " + s / 4 + " 0 l 0 " + s / 4
							}(size);
						case "waves":
							return function(s) {
								return "M 0 " + s / 2 + " c " + s / 8 + " " + -s / 4 + " , " + s * 3 / 8 + " " + -s / 4 + " , " + s / 2 +
									" 0\nc " + s / 8 + " " + s / 4 + " , " + s * 3 / 8 + " " + s / 4 + " , " + s / 2 + " 0 M " + -s / 2 +
									" " + s / 2 + "\nc " + s / 8 + " " + s / 4 + " , " + s * 3 / 8 + " " + s / 4 + " , " + s / 2 + " 0 M " +
									s + " " + s / 2 + "\nc " + s / 8 + " " + -s / 4 + " , " + s * 3 / 8 + " " + -s / 4 + " , " + s / 2 +
									" 0"
							}(size);
						case "woven":
							return function(s) {
								return "M " + s / 4 + "," + s / 4 + "l" + s / 2 + "," + s / 2 + "M" + s * 3 / 4 + "," + s / 4 + "l" + s /
									2 + "," + -s / 2 + "\nM" + s / 4 + "," + s * 3 / 4 + "l" + -s / 2 + "," + s / 2 + "M" + s * 3 / 4 + "," +
									s * 5 / 4 + "l" + s / 2 + "," + -s / 2 + "\nM" + -s / 4 + "," + s / 4 + "l" + s / 2 + "," + -s / 2
							}(size);
						case "crosses":
							return function(s) {
								return "M " + s / 4 + "," + s / 4 + "l" + s / 2 + "," + s / 2 + "M" + s / 4 + "," + s * 3 / 4 + "l" + s /
									2 + "," + -s / 2
							}(size);
						case "caps":
							return function(s) {
						...
						case "hexagons":
							return function(s) {
								...
								return "M " + s + ",0 l " + s + ",0 l " + s / 2 + "," + s * Math.sqrt(3) / 2 + "\nl " + -s / 2 + "," + s *
									Math.sqrt(3) / 2 + " l " + -s + ",0\nl " + -s / 2 + "," + -s * Math.sqrt(3) / 2 + " Z M 0," + s * Math.sqrt(
				...
				paths = function(sel) {
					...
					id = rand();
					g = sel.append("defs").append("pattern").attr("id", id).attr("patternUnits", "userSpaceOnUse").attr("width",
						size * width).attr("height", size * height);
					if (background) {
						g.append("rect").attr("width", size * width).attr("height", size * height).attr("fill", background)
					}
					return g.append("path").attr("d", path).attr("fill", fill).attr("stroke-width", strokeWidth).attr(
						"shape-rendering", shapeRendering).attr("stroke", stroke).attr("stroke-linecap", "square")
				};
