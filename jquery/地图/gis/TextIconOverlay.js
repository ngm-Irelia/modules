...
(function () {
    ...
    var _IMAGE_PATH = '../../../image/gis/circle';
    /**
    ...
    var TextIconOverlay =
         ...
         */
        BMapLib.TextIconOverlay = function(position, text, options,areaMarker){
            this._position = position;
            this._text = text;
            this._areaMarker = areaMarker;
    ...
    TextIconOverlay.prototype.initialize = function(map){
        ...
        divattr.value = "divJH";
        this._domElement.setAttributeNode(divattr);

        this._updateCss();
        this._updateText();
        this._updatePosition();
        this._updateAreaMarker();
    ...
    /**
     *设置该覆盖物的marker有多少。
     *@param {Point}  areaMarker  要设置的经纬度坐标。
     *@return 无返回值。
     */
    TextIconOverlay.prototype.setAreaMarker = function (areaMarker) {
        if( areaMarker ){
            this._areaMarker =  areaMarker;
            this._updateAreaMarker();
        }
     ...
     *@return 无返回值。
     */
    TextIconOverlay.prototype._updateAreaMarker = function(){

        var markerType = [];
        for(var i=0;i<this._areaMarker.length;i++){
            var type = this._areaMarker[i].type;
            var haveSign = false;
        ...
        var svg = d3.select(this._domElement)
            .append("svg")
            .attr("width",width)
            .attr("height",height)
        ...
        var color = ["#0099FF","#CCCC33","#003366","#99CCFF","#663366","#996633","#663300","#993366","#CCCCFF","#FFFF99"];
