...
(function() {
  ...
  function CurveLine(twoId,sign, points, opts) {
    var self = this;
    var twoId = twoId;
    var sign = sign;
    var curvePoints = getCurvePoints(points,sign);
    ...
    //   }
    // });
    polyline.twoId = twoId;
    polyline.time = twoId.time;
    polyline.sign = "polyline";
    ...
    polyline.enableEditing = function () {
      ...
      if (self.map) {
        self.disableEditing();
        if(sign=="txcurve"){
          for (var i = 1; i < self.cornerPoints.length; i++) {
            //我要的是 中间的点
            let getCenterByCurvepoints = self.getPath();
            let centerNum = parseInt(getCenterByCurvepoints.length/2);
            let centerP = getCenterByCurvepoints[centerNum];
            var marker = new BMap.Marker(centerP, {
                icon: new BMap.Icon('../../../image/gis/circle-red.svg', new BMap.Size(14,14)),
                //enableDragging: true,
                raiseOnDrag: true
            });
            ...
            var content = "<div class='address_indiv'>"+
                "时间：<span class='address_div' >"+ twoId.time +"</span> </br>"+
                "起点：<span class='address_div' >"+ "["+twoId.firstPoint.lng+" , "+twoId.firstPoint.lat+"]" +"</span> </br>"+
                "终点：<span class='address_div' >"+ "["+twoId.lastPoint.lng+" , "+twoId.lastPoint.lat+"]" + "</span> </br>"+
            ...
            label.setStyle({display: "none",backgroundColor: "rgba(0,0,0,0)",border:"#1E262F"});
            ...
            marker.addEventListener("click",function(){
                ...
                setTimeout(function(){
                    $(".address_indiv").css("width","180px");
                    $(".address_indiv").css("height","80px");
                },100);
            })
            ...
            for (var i = 1; i < self.cornerPoints.length; i++) {
              ...
              /*marker.addEventListener('dragend', function(){
               ...
               self.setPath(curvePoints);
                ...
                var content = "<div class='address_indiv'>"+
                    "地址：<div class='address_div' >"+ twoId.address +"</div> "+
                    "<div class='dbl_topleft'></div><div class='address_bottomright'></div></div>";
      ...
      self.isEditing = true;
        ...
        //self.map.disableDoubleClickZoom(true);   //禁用双击放大
    }
   ...
   * 根据弧线的坐标节点数组
   */
  function getCurvePoints(points,sign) {
    var curvePoints = [];
    for (var i = 0; i < points.length - 1; i++) {
      var p = getCurveByTwoPoints(points[i], points[i + 1],sign);
      if (p && p.length > 0) {
   ...
   * @param Point 终点
   */
  function getCurveByTwoPoints(obj1, obj2,sign) {
