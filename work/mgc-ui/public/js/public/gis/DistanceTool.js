...
(function() {
    ...
    DistanceTool.prototype.open = function(){
        ...
        var distClick = function(e) {
            ...
            $("img[src='http://api.map.baidu.com/images/mapctrls.png']").css("max-width","inherit");//解决和epm-ui冲突问题
            //$("span[class='BMap_Marker'] img").css("max-width","inherit");//解决和epm-ui冲突问题
            /**
    ...
    DistanceTool.prototype._processLastOp = function() {
        ...
        $("img[src='http://api.map.baidu.com/images/mapctrls.gif']").css("max-width","inherit");
        //$("span[class='BMap_Marker'] img").css("max-width","inherit"); // 关闭按钮排除 epm-ui css冲突
        disObj.closeBtn.setTitle("清除本次测距");
        // 点击关闭按钮，绑定关闭按钮事件
        disObj.closeBtn.addEventListener("click", function(e){
            $("svg[type='system']").css("cursor","pointer"); //新加，关闭事件，鼠标恢复
            // 关闭本次测距，清除相关存储和变量
