...
(function() {
    ...
    DrawingManager.prototype._initialize = function(map, opts) {
        ...
        /**
         * 当前的绘制模式, 默认是绘制点BMAP_DRAWING_MARKER
         * @private
         * @type {DrawingType}
         */
        this._drawingType = opts.drawingMode || "hander";
    ...
    DrawingManager.prototype._bindMarker = function() {
        ...
        var clickAction = function (e) {
            ...
            var label = new BMap.Label(e.point,{offset:new BMap.Size(20,-10)});
    ...
    DrawingManager.prototype._bindCircle = function() {
        ...
        var startAction = function (e) {
            ...
            circleSize = circleSize+1; //圆的数量加一
            var sizeData = circleSize;
            centerPoint = e.point;
            ...
            centerMarker = new BMap.Marker(centerPoint, {
                icon: new BMap.Icon('/image/gis/circle.png', new BMap.Size(10,10)),
                //enableDragging: true,
                raiseOnDrag: true
            });
            //修改，添加label作为参考
            var label = new BMap.Label("0",{offset:new BMap.Size(20,-20)});
            label.setStyle({backgroundColor: "#D7EFFF",border:"#D7EFFF"});
            var labelHtml = '<div id="circle_radius'+circleSize+'idDiv"> '+
                                '<div id="circlediv'+circleSize+'">R: &nbsp<input id="circle_radius'+circleSize+'id" class="input_r" type="text" value="0" autofocus />km</div>'+
                            '</div>';
            label.setContent(labelHtml)
            centerMarker.setLabel(label);
            centerMarker.circle = circle;
            map.addOverlay(centerMarker);
            $('#circlediv'+circleSize).css("display","block");
             rrmarker = new BMap.Marker(centerPoint, {
                icon: new BMap.Icon('/image/gis/circle.png', new BMap.Size(16,16))
             });
            rrmarker.centerPoint = centerPoint;
            rrmarker.circle = circle;
            rrmarker.circleSize = circleSize;
            //修改，添加label作为参考
            //var rrlabel = new BMap.Label("gz",{offset:new BMap.Size(20,0)});
            //label.setStyle({backgroundColor: "#D7EFFF",border:"#D7EFFF",width:'20px'});
            //marker.setLabel(label);
            map.addOverlay(rrmarker);
            rrmarker.setAnimation(null);
            rrmarker.enableDragging();
            rrmarker.addEventListener('dragstart',startMoveMarker);
            rrmarker.addEventListener('dragging',inMoveMarker);
            rrmarker.addEventListener('dragend',endMoveMarker);

            $('#circle_radius'+circleSize+'id').css("width","10px");
            $('#circle_radius'+circleSize+'id').parent().css("width","20px");

            $(".input_r").bind("click",function (e) {
                e.preventDefault();
                e.stopPropagation();
                $(this).focus();
            })

            $('#circle_radius'+sizeData+'id').keydown(function(event){

                if (event.keyCode == 13) {
                    var text_data = $(this).val();//获取当前文本框的值
                    /*var len = text_data.indexOf('m');
                    var numData = text_data.slice(0,len);
                    circle.setRadius(numData);*/
                    circle.setRadius(parseFloat(text_data)*1000);

                    if(circle.hasOwnProperty("ia")){
                        var ialen = circle.ia.length;
                        var numia = parseInt(ialen*3/4);
                        rrmarker.setPosition(circle.ia[numia]);
                    }

                    var radius = circle.xa+"m";
                    //var radius = parseInt(circle.xa);

                    var lon = circle.point.lng;
                    var lat = circle.point.lat;

                    var mapMarkers = getMapMarkers();
                    var mapMarkersid = [];
                    for(var k=0;k<mapMarkers.length;k++){
                        if(mapMarkers[k].hasOwnProperty("addnode")){
                    ...
                    circle.radius = circle.xa;
                    circle.centerPoint = [lon,lat];
                    circle.type = "circle";

                    if(!mapMarkersid){
                        circle.multipleEntity = [];
                        return '';
                    }
                    $.ajax({
                        ...
                        success: function (data) {
                            if(data.magicube_interface_data.hasOwnProperty("entity")){
                                ...
                                circle.multipleEntity = dataMultipleEntity;
            ...
            $('.circledivr').bind("click",function () {
        ...
        var startMoveMarker = function (e,ee) { }
        var inMoveMarker = function (e,ee,pix,point) {
            circle.setRadius(me._map.getDistance(this.centerPoint, e.point));

            /*var labelHtml = '<div><input id="circle_radius'+circleSize+'id" className="circle_radius" type="text" value="'+ parseInt(me._map.getDistance(this.centerPoint, e.point)) +'m"/></div>';
            centerMarker.getLabel().setContent(labelHtml);*/
            $('#circle_radius'+this.circleSize+'id').val(parseFloat(parseInt(me._map.getDistance(this.centerPoint, e.point))/1000));
            var text_length = $('#circle_radius'+this.circleSize+'id').val().length;//获取当前文本框的长度
            var current_width = parseInt(text_length)*7 +5;//该16是改变前的宽度除以当前字符串的长度,算出每个字符的长度
            $('#circle_radius'+this.circleSize+'id').css("width",current_width+"px");
            $('#circle_radius'+this.circleSize+'id').css("-webkit-user-select","auto !important");
            $('#circle_radius'+this.circleSize+'id').css("user-select","auto !important");
            $('#circle_radius'+this.circleSize+'id').parent().css("width",current_width+40+"px");
        }
        var endMoveMarker = function (e,ee,pix,point) {

            //var radius = parseFloat(circle.xa);
            var radius = circle.xa+"m";
            var lon = circle.point.lng;
            var lat = circle.point.lat;
        ...
        var moveAction = function(e) {
            ...
            $('#circle_radius'+circleSize+'id').val(parseFloat(parseInt(me._map.getDistance(centerPoint, e.point))/1000));
            var textval = $('#circle_radius'+circleSize+'id').val();
            var text_length = 0;//获取当前文本框的长度
            if(textval){
                text_length = textval.length;
            }else{
                text_length = 5;
            }
            var current_width = parseInt(text_length)*7 +5;//该16是改变前的宽度除以当前字符串的长度,算出每个字符的长度
            $('#circle_radius'+circleSize+'id').css("width",current_width+"px");
            $('#circle_radius'+circleSize+'id').parent().css("width",current_width+40+"px");
            $('#circle_radius'+circleSize+'id').css("-webkit-user-select","auto !important");
            $('#circle_radius'+circleSize+'id').css("user-select","auto !important");
            rrmarker.setPosition(e.point);

            if(circle.hasOwnProperty("ia")){
                var ialen = circle.ia.length;
                var numia = parseInt(ialen/4);
        ...
        var endAction = function (e) {
            //ngm在这里和后台交互  参数：  圆心 centerPoint  半径 me._map.getDistance(centerPoint, e.point)
            //circle.enableEditing();
            var text_length = $('#circle_radius'+circleSize+'id').val().length;//获取当前文本框的长度
            var current_width = parseInt(text_length)*7 +5;//该16是改变前的宽度除以当前字符串的长度,算出每个字符的长度
            $('#circle_radius'+circleSize+'id').css("width",current_width+"px");
            $('#circle_radius'+circleSize+'id').parent().css("width",current_width+40+"px");
            ...
            //var radiusstr = parseInt(circle.xa)/1000;
            var radius = circle.xa+"m";
            //var radius = radiusstr+"km";
            var lon = circle.point.lng;
            var lat = circle.point.lat;
    ...
    DrawingManager.prototype._bindPolylineOrPolygon = function() {
        ...
        var dblclickAction = function (e) {
            ...
            //ngm在这获得所有的point坐标信息 ，传给后端 参数：points
            ...
            var lenUse = usePoint.length;
            var lonUse = [];
            var latUse = [];
            if(lenUse>3){
                for(var i=0;i<usePoint.length-1;i++){
                    lonUse.push(parseFloat(usePoint[i].lng));
                    latUse.push(parseFloat(usePoint[i].lat));
                }
                lonUse.push(parseFloat(usePoint[0].lng));
                latUse.push(parseFloat(usePoint[0].lat));
            }
            ...
            overlay.points = usePoint;
            overlay.lonUse = lonUse;
            overlay.latUse = latUse;
            overlay.type = "polygon";

            if(!mapMarkersid){
                overlay.multipleEntity = [];
                return '';
            }
            ...
            $.ajax({
                ...
                success: function (data) {
                    if(data.magicube_interface_data.hasOwnProperty("entity")){
                        ...
                        overlay.multipleEntity = dataMultipleEntity;
                        changeMarkerColor(data.magicube_interface_data.entity);
    ...
    DrawingManager.prototype._bindRectangle = function() {
         ...
         */
        var endAction = function (e) {
            // ngm在这获得矩形的起点startPoint 终点getEndPoint  传给后端

            var lonUse = [];
            var latUse = [];
            var ngmpoints = [];
            if(startPoint&&getEndPoint){
                lonUse.push(parseFloat(startPoint.lng));
                latUse.push(parseFloat(startPoint.lat));
                ngmpoints.push([parseFloat(startPoint.lng),parseFloat(startPoint.lat)]);
                lonUse.push(parseFloat(startPoint.lng));
                latUse.push(parseFloat(getEndPoint.lat));
                ngmpoints.push([parseFloat(startPoint.lng),parseFloat(getEndPoint.lat)]);
                lonUse.push(parseFloat(getEndPoint.lng));
                latUse.push(parseFloat(getEndPoint.lat));
                ngmpoints.push([parseFloat(getEndPoint.lng),parseFloat(getEndPoint.lat)]);
                lonUse.push(parseFloat(getEndPoint.lng));
                latUse.push(parseFloat(startPoint.lat));
                ngmpoints.push([parseFloat(getEndPoint.lng),parseFloat(startPoint.lat)]);
                lonUse.push(parseFloat(startPoint.lng));
                latUse.push(parseFloat(startPoint.lat));
            }
            ...
            polygon.points = ngmpoints;
            polygon.lonUse = lonUse;
            polygon.latUse = latUse;
            polygon.type = "polygon";

            if(!mapMarkersid){
                polygon.multipleEntity = [];
                return '';
            }
            ...
            $.ajax({
                ...
                success: function (data) {
                    if(data.magicube_interface_data.hasOwnProperty("entity")){
                        ...
                        polygon.multipleEntity = dataMultipleEntity;
                        changeMarkerColor(data.magicube_interface_data.entity);
                    }
                }
            });
    ...
    function DrawingTool(drawingManager, drawingToolOptions) {
        ...
        //默认所有工具栏都显示 BMAP_DRAWING_MARKER,BMAP_DRAWING_POLYLINE,
    ...
    DrawingTool.prototype._generalHtml = function(map){
        ...
        var getItem = function(className, drawingType) {
            if(drawingType=="hander"){
                return '<a class="' + className + '" drawingType="' + drawingType + '" href="javascript:void(0)" title="' + tips[drawingType] + '" onfocus="this.blur()"> 拖拽地图</a><br>';
            }else if(drawingType=="marker"){
                return '<a class="' + className + '" drawingType="' + drawingType + '" href="javascript:void(0)" title="' + tips[drawingType] + '" onfocus="this.blur()">span className="icon-square-layout"></span>加点</a><br>';
            }else if(drawingType=="circle"){
                return '<a class="' + className + '" drawingType="' + drawingType + '" href="javascript:void(0)" title="' + tips[drawingType] + '" onfocus="this.blur()"> 圆形选区</a><br>';
            }else if(drawingType=="polyline"){
                return '<a class="' + className + '" drawingType="' + drawingType + '" href="javascript:void(0)" title="' + tips[drawingType] + '" onfocus="this.blur()"><span className="icon-polygon-o"></span>画折线</a><br>';
            }else if(drawingType=="polygon"){
                return '<a class="' + className + '" drawingType="' + drawingType + '" href="javascript:void(0)" title="' + tips[drawingType] + '" onfocus="this.blur()"> 多边选区</a><br>';
            }else if(drawingType=="rectangle"){
                return '<a class="' + className + '" drawingType="' + drawingType + '" href="javascript:void(0)" title="' + tips[drawingType] + '" onfocus="this.blur()"> 方形选区</a><br>';
            }

            return '<a class="' + className + '" drawingType="' + drawingType + '" href="javascript:void(0)" title="' + tips[drawingType] + '" onfocus="this.blur()">drawingType</a>';
        }

        var html = [];
        html.push(getItem("BMapLib_box BMapLib_hander icon-arrows", "hander"));
        for (var i = 0, len = this.drawingModes.length; i < len; i++) {
            if(this.drawingModes[i] == "circle"){
                var classStr = 'BMapLib_box BMapLib_' + this.drawingModes[i] +' icon-circle-o';
            }else if(this.drawingModes[i] == "polygon"){
                var classStr = 'BMapLib_box BMapLib_' + this.drawingModes[i] +' icon-polygon-o';
            }else if(this.drawingModes[i] == "rectangle"){
                var classStr = 'BMapLib_box BMapLib_' + this.drawingModes[i] +' icon-square-o-big';
            }else if(this.drawingModes[i] == "hander"){
                var classStr = 'BMapLib_box BMapLib_' + this.drawingModes[i] +' icon-arrows';
            }else {
                var classStr = 'BMapLib_box BMapLib_' + this.drawingModes[i] +' icon-arrows';
            }
            //var classStr = 'BMapLib_box BMapLib_' + this.drawingModes[i] +' icon-arrows';
