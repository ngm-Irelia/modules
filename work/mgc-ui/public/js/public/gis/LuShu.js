...
(function() {
     ...
     */
     BMapLib.LuShu = function(name, map, Id, path, opts) {
        if (!path || path.length < 1) {
            return;
        }
        this._map = map;
        this._id = Id;//ID用来结束时删除图标的的唯一标识
        //存储一条路线
        this._path = path;
        this.lushuName = name; //判断是哪一个路书
        //移动到当前点的索引
        this.i = 0;
        this.c = 0;//次数
        //控制暂停后开始移动的队列的数组
        ...
        _move: function(initPos,targetPos,effect) {
            ...
            //两点之间当前帧数大于总帧数的时候，则说明已经完成移动
	            if (currentCount >= count) {
                    ...
                    /*var settargetPoslng = Math.floor(targetPos.lng * 100000);
                    var settargetPoslat = Math.floor(targetPos.lat * 100000);
                    var setPoslng = Math.floor(pos.lng * 100000);
                    var setPoslat = Math.floor(pos.lat * 100000);
                    if((Math.abs(settargetPoslng-setPoslng)<=1)&&Math.abs(settargetPoslat-setPoslat)<=1){
                    }*/
        ...
        */
        ...
        _moveNext: function(index) {
            ...
            if (index < this._path.length - 1) {
                ...
                //ngm 找到了，在这删除   -- 如果是动画效果 改为 重新开始
                this._map.removeOverlay(this._marker);

                for(let l=0;l<lushuNum.length;l++){
                  if(lushuNum[l].name === this.lushuName){
                    lushuNum[l].size = lushuNum[l].size+1;//这是个重点，判断值
                  }
                }

                //window.lushunum = window.lushunum+1;//这是个重点，判断值
