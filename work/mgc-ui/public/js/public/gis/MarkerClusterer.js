...
(function(){
    ...
    var MarkerClusterer =
        ...
        BMapLib.MarkerClusterer = function(map, options){
            ...
            var that = this;
            this._map.addEventListener("zoomend",zoomendJH,false);
            this._map.addEventListener("moveend",moveendJH,false);

            window.setZoomendJH = function (){
                that._redraw();
            }
    ...
    /**
     * 把一个标记添加到要聚合的标记数组中--把所有marker放在一起，添加的时候使用了indexOf去重呢
     ...
     * 停止聚合
     */
    MarkerClusterer.prototype.stopClusterer = function () {
        //this._map.removeEventListener("moveend",moveendJH,false);
        //this._map.removeEventListener("zoomend",zoomendJH,false);
        ...
        for(var i = 0;i<this._markers.length; i++){
            ...
            var label = new BMap.Label(marker.baseMsg.name,{offset:new BMap.Size(20,-10)});
            label.setStyle({backgroundColor: "#5B7085",border:"#D7EFFF"});
            marker.setLabel(label);
            this._map.addOverlay(marker);//点其实被全部去掉了。然后在这又被加回来
        }

        this._markers = [];//置空marker数组
     ...
     * @return 无返回值
     */
    MarkerClusterer.prototype._returnMarkers = function(){
    ...
    MarkerClusterer.prototype._createClusters = function(){
        ...
        for(var i = 0, marker; marker = this._markers[i]; i++){
            if(!marker.isInCluster && extendedBounds.containsPoint(marker.getPosition()) ){
                this._addToClosestCluster(marker);//判断点的距离？？开始聚合
            }
        }
    };

    /**
     * 根据标记的位置，把它添加到最近的聚合中  --  在这里还 需要判断距离是否符合要求？还没有聚合？
    ...
    MarkerClusterer.prototype._addToClosestCluster = function (marker){
        ...
        if (clusterToAddTo && clusterToAddTo.isMarkerInClusterBounds(marker)){
            ...
            var cluster = new Cluster(this);//聚合体不存在，新加聚合体
    ...
    function Cluster(markerClusterer){
        ...
        this._clusterMarker =
            new BMapLib.TextIconOverlay(this._center, this._markers.length, {"styles":this._markerClusterer.getStyles()} ,this._markers);
    ...
    Cluster.prototype.addMarker = function(marker){
        ...
        if(len < this._minClusterSize ){
            var label = new BMap.Label(marker.baseMsg.name,{offset:new BMap.Size(20,-10)});
            label.setStyle({backgroundColor: "#5B7085",border:"#D7EFFF"});//aaaaaaaaaaa
            marker.setLabel(label);
            this._map.addOverlay(marker);//点其实被全部去掉了。然后在这又被加回来

            /*var allOverlays = this._map.getOverlays();
            for(var i=0;i<allOverlays.length;i++){
                if(allOverlays[i].hasOwnProperty("polylineid")&&marker.hasOwnProperty("id")){
                    if(allOverlays[i].sourceid==marker.id){ //移动线 而不是 删除线
                        allOverlays[i].setPositionAt(0,this._center);
                    }
                    if(allOverlays[i].targetid==marker.id){  //移动线 而不是 删除线
                        allOverlays[i].setPositionAt(1,this._center);
                    }
                }
            }*/
            ...
            for (var k = 0; k < len; k++) {
                this._markers[k].getMap() && this._map.removeOverlay(this._markers[k]);//在这里删除了该marker 同时应该 移动相关的线
                /*var allOverlays = this._map.getOverlays();
                for(var i=0;i<allOverlays.length;i++){
                    if(allOverlays[i].hasOwnProperty("polylineid")&&this._markers[k].hasOwnProperty("id")){
                        if(allOverlays[i].sourceid==this._markers[k].id){
                            //移动线 而不是 删除线
                            allOverlays[i].setPositionAt(0,this._center);
                        }
                        if(allOverlays[i].targetid==this._markers[k].id){
                            //移动线 而不是 删除线
                            allOverlays[i].setPositionAt(1,this._center);
                        }
                    }
    ...
    Cluster.prototype.updateClusterMarker = function () {
        ...
        this._clusterMarker.setAreaMarker(this._markers);//把选区内点给 图形 使用d3去画了
    ...
    Cluster.prototype.addLabel = function (marker) {
        ...
        var content = '<span style="color:'+this._styles[0].backgroundColor+'"><i class="fa fa-map-marker"></i></span>'+'<p style="padding:0px 13px;text-align:center;margin-top:5px;">这是一个点</p>';
