...
(function() {
    ...
    DrawingManager.prototype._bindCircle = function() {
        ...
        var startAction = function (e) {
            ...
            centerMarker = new BMap.Marker(centerPoint, {
                icon: new BMap.Icon('http://api.map.baidu.com/library/CurveLine/1.5/src/circle.png', new BMap.Size(10,10)),
                //enableDragging: true,
                raiseOnDrag: true
            });
            //修改，添加label作为参考
            ...
            var labelHtml = '<div id="circle_radius'+circleSize+'idDiv" class="labelInput"> '+
        ...
        var moveAction = function(e) {
            ...
            /*var labelHtml = '<div><input id="circle_radius'+circleSize+'id" type="text" value="'+ parseInt(me._map.getDistance(centerPoint, e.point)) +'m"/></div>';
            centerMarker.getLabel().setContent(labelHtml);*/
        ...
        var endAction = function (e) {
            ...
            $('#circle_radius'+circleSize+'id').bind("click",function (eve) {
                eve.preventDefault();
                eve.stopPropagation();
                $(this).focus();
                //document.getElementById('circle_radius1id').focus();
            })

            $('#circle_radius'+circleSize+'id').keydown(function(event){
    ...
    function DrawingTool(drawingManager, drawingToolOptions) {
        ...
        //默认所有工具栏都显示 BMAP_DRAWING_MARKER,BMAP_DRAWING_POLYLINE,BMAP_DRAWING_POLYGON,
    ...
    DrawingTool.prototype._generalHtml = function(map){
        ...
        var getItem = function(className, drawingType) {
            if(drawingType=="hander"){//<img src="../image/gis/gis_drafting2.svg"/>
                return '<a class="' + className + '" drawingType="' + drawingType + '" href="javascript:void(0)" title="' + tips[drawingType] + '" onfocus="this.blur()"></a><br>';
    ...
    DrawingTool.prototype._bind = function(panel){
        ...
        baidu.on(this.panel, 'mouseover', function (e) {
            /*var target = baidu.getTarget(e);
            var drawingType = target.getAttribute('drawingType');

            me.setStyleByDrawingMode(drawingType);*/
        });
        baidu.on(this.panel, 'mouseout', function (e) {
            /*var target = baidu.getTarget(e);
            var drawingType = target.getAttribute('drawingType');
...
//ngm img 控制
        var boximgs = this.panel.getElementsByTagName("a");
        for (var i = 0, len = boximgs.length; i < len; i++) {
            var boximg = boximgs[i];
            if (boximg.getAttribute('drawingType') == drawingType) {
                var classStr = 'BMapLib_box BMapLib_' + drawingType +'_hover icon-square-o-big';
                if(drawingType == "circle"){
                    var classStr = 'BMapLib_box BMapLib_' + drawingType +'_hover icon-circle-o';
                } else if(drawingType == "rectangle"){
                ...
                //box.className = box.className.replace(/_hover/, "");
                //boximg.src = boximg.src.replace(/-hover/, "");
            }
        }
